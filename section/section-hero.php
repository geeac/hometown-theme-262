...
<div class="carousel bg" data-items="1" data-single-item="true" data-smart-speed="500" data-dots="false" data-animate-out="fadeOut">
<?php 
	if(is_array($properties))
	foreach($properties as $property):
		
	$img = wp_get_attachment_image_src( get_post_thumbnail_id($property['stack_title']), 'slide-full');
?>
	<div class="item" style="background-image: url(<?php echo $img[0]; ?>);"></div>
<?php endforeach; ?>
</div>

<div class="carousel" data-items="1" data-single-item="true" data-smart-speed="500" data-link=".carousel.bg" data-autoplay="<?php echo $property_slide_autoplay; ?>" data-autoplay-timeout="<?php echo $property_slide_timeout; ?>" data-autoplay-hover-pause="true" data-loop="true">
<?php 
	global $post;
	if(is_array($properties))
	foreach($properties as $property):
...
?>
<div class="map-wrap" id="map-12" style="height: <?php echo $map_height; ?>px;" data-zoom="<?php echo $map_zoom; ?>">
<?php 
...
?>	
	<div data-active="" data-latitude="<?php echo esc_attr($location[1]); ?>" data-longitude="<?php echo esc_attr($location[2]); ?>" data-content='<div class="card"><div class="img-wrap"><a href="<?php echo get_permalink($property->ID); ?>"><img src="<?php echo $thumb[0]; ?>" /></a><div class="badge"><div class="status"><ul class="meta-list"><?php echo get_the_term_list($property->ID, 'status', '<li>', '</li><li>', '</li>'); ?></ul></div><?php if($price): ?><div class="price small"><?php echo nt_currency($price, $per); ?></div><?php endif; ?></div></div><div class="inner"><div class="title"><a href="<?php echo get_permalink($property->ID); ?>"><?php echo esc_attr(get_the_title($property->ID)); ?></a></div><ul class="meta-list"><?php echo get_the_term_list($property->ID, 'location', '<li>', '</li><li>', '</li>'); ?></ul></div></div>'></div>
<?php endforeach; ?>
</div>
<?php endif; ?>
...
<div class="hero slide-hero">
	...
	<?php 
		...
		$img = wp_get_attachment_image_src( $slide['bg_image'], 'slide-full');
	?>
		<div class="item" style="background-image: url(<?php echo $img[0]; ?>);"></div>
	<?php endforeach; ?>
	</div>

	<div class="carousel carousel-content" data-items="1" data-single-item="true" data-smart-speed="500" data-link=".carousel.bg" data-autoplay="<?php echo $slide_autoplay; ?>" data-autoplay-timeout="<?php echo $slide_timeout; ?>" data-autoplay-hover-pause="true" data-loop="true">
		<?php 
			if(is_array($slides))
			foreach($slides as $slide):
		?>
