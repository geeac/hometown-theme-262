...
class nt_input_tool {
	...
	function nt_input_tool( $options, $config ) {
	...
	function textarea( $option ) {
		...
		if( $option['id'] == 'theme_export_option' ) $value = base64_encode( serialize( nt_get_option() ) );
...
'https://www.googleapis.com/webfonts/v1/webfonts?key=AIzaSyALn_gIORMu90pZJhO4-z5xULIWH9Lce-A';
	...
	function location( $option ) {
		...
		echo '<div class="location-picker" style="height: 300px;" data-latitude="'.$value[1].'" data-longitude="'.$value[2].'" data-location="'.$value[0].'"></div>';
	}
